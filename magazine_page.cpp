...
magazine_page::magazine_page(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::magazine_page)
{
    ui->setupUi(this);
...
void magazine_page::on_back_button_clicked()
{
    close();
...
void magazine_page::on_clear_button_clicked()
{
    close();
...
void magazine_page::prepro_data(QString filename) {
    ...
    while (true){
        ...
        info = data.mid(3);
        if (header == QString("%0 ")){
            //Check whether the reference belong to a magzine
            //if not, pop up a warning window
            if (info.indexOf("Magazine") == string::npos){
                int warn_ret = QMessageBox::warning(this,tr("warn"),tr("Your loading reference does not belong to a magazine"),QMessageBox::Abort);
            ...
            ui->magazine_title_line->setText(trim(info));
        }
...
void magazine_page::on_load_button_clicked()
{
...
void magazine_page::on_editor_check_clicked(bool checked)
{
    if (checked){
...
void magazine_page::on_translator_check_clicked(bool checked)
{
    if (checked){
...
void magazine_page::on_author_checkBox_clicked(bool checked)
{
    if (checked){
...
void magazine_page::on_get_citation_button_clicked()
{
    ...
    if (ui->reference_style->currentText() == QString("APA")){
        ...
        if (ui->English_checkBox->isChecked()){
            ...
            //get bibliography
            bibliography = get_APA_EN_bibliography_magazine(last_name_base,first_name_base,ui->date_line->text(),editor_last_name_base,
                                                  editor_first_name_base,translator_last_name_base,translator_first_name_base,ui->article_title_line->text(),
                                                           ui->magazine_title_line->text(),ui->start_page_line->text(),ui->end_page_line->text());
        } else {
            //Chinese version
            ...
            //get bibliography
            bibliography = get_APA_CN_bibliography_magazine(last_name_base,ui->date_line->text(), editor_last_name_base, translator_last_name_base,
                                                   ui->article_title_line->text(),ui->magazine_title_line->text(),
        ...
        if (ui->English_checkBox->isChecked()){
            ...
            //get bibliography
            bibliography = get_MLA_EN_bibliography_magazine(last_name_base,first_name_base,ui->date_line->text(),editor_last_name_base,
                                                   editor_first_name_base,translator_last_name_base,translator_first_name_base,
                                                           ui->article_title_line->text(),ui->magazine_title_line->text(),ui->start_page_line->text(),ui->end_page_line->text());
            ...
            bibliography = get_MLA_CN_bibliography_magazine(last_name_base,ui->date_line->text(),
